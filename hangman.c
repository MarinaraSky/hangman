    ...
    char *secretWord)
{
    ...
    getStats(&win, &loss, &totalGuess, &totalTime, &currentStreak,
             &highStreak);
    ...
    saveStats(&win, &loss, &totalGuess, &totalTime, &currentStreak,
              &highStreak);
}
    ...
    char *filename)
{
    ...
    while ((readWordSize =
            getline(&readWord, &readWordLength, wordList)) != -1)
    {
    ...
    int *highStreak)
{
    ...
    fprintf(stats, "%d %d %d %d %d %d", *win, *loss, *totalGuess, *totalTime,
